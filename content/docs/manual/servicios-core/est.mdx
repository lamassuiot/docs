...
- **Servicio webhook accesible** (solo si `AuthMode` incluye `EXTERNAL_WEBHOOK`): el servicio externo de autorización debe estar operativo y ser alcanzable desde el servidor Lamassu antes de activar el modo.

Conviene separar tres bloques de configuración:
...
Los endpoints EST exigen siempre HTTPS. Cuando el servidor está configurado con mTLS en la capa HTTPS, negocia únicamente **TLS 1.2** como versión máxima; en despliegues sin mTLS puede negociar hasta TLS 1.3. Un cliente cuyo `MinVersion` esté fijado a TLS 1.3 fallará el handshake contra un endpoint con mTLS habilitada. En Go, fija `tls.Config{MinVersion: tls.VersionTLS12}`.
...
- **`EXTERNAL_WEBHOOK`**: Lamassu delega la decisión de autorización en un servicio HTTP externo. El servidor EST construye una petición `POST` al webhook configurado con un cuerpo JSON que reproduce el contexto de la inscripción.
  ...
  El valor del campo `csr` es la **codificación base64 del PEM** del CSR (no el DER, y no el PEM en claro). El webhook debe responder con `2xx` y `{"authorized": true}` para continuar; cualquier otra respuesta aborta el enrolamiento con `external webhook denied enrollment`.

  <Callout type="warn" title="Degradación silenciosa en re-enrollment">
  El webhook **solo se invoca en `/simpleenroll`**. En `/simplereenroll`, el modo `EXTERNAL_WEBHOOK` degrada silenciosamente a `NO_AUTH`: si el cliente no presenta un certificado cliente, el reenroll continúa sin autenticación, con un aviso en el log (`allowing reenroll: using NO AUTH mode`). La decisión de autorización del webhook queda fuera del flujo de renovación. Un DMS configurado con `EXTERNAL_WEBHOOK` no debe usarse en producción para flujos de reenroll a menos que se controle la red de acceso al endpoint. Lamassu IoT no compensa esta degradación con ningún otro control.
  </Callout>
