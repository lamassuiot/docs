...
description: Guía del protocolo EST en Lamassu IoT, con la configuración del DMS para el administrador PKI y la referencia de endpoints para el integrador de dispositivos.
---

# EST: Enrollment over Secure Transport

Lamassu IoT implementa el protocolo **EST (Enrollment over Secure Transport)** descrito en el RFC 7030 como su único mecanismo de inscripción de certificados para dispositivos. Cada DMS configurado en la plataforma realiza el rol de autoridad de registro (RA) frente al cliente EST: expone los endpoints, valida al dispositivo y aplica sus propias políticas de autenticación, registro y renovación. Cuando la solicitud se aprueba, quien emite y firma criptográficamente el certificado es la CA asociada a ese DMS; el DMS coordina el proceso, pero no actúa como entidad firmante.
...
*Esta sección está dirigida al administrador PKI.*

Antes de entrar en el detalle, conviene separar tres bloques de configuración:

- Cómo se realizará el enrollment inicial, incluyendo el modo de alta (`JITP` o `PRE_REGISTRATION`) y la autenticación del cliente.
...
- **Tags**: se aplican a cada dispositivo creado automáticamente por este DMS. Son opcionales y solo tienen efecto en modo JITP.
- **Icon**: se aplica a cada dispositivo creado automáticamente por este DMS. Es opcional y solo tiene efecto en modo JITP.
...
- **HTTP Method**: método HTTP utilizado para invocar el webhook.
- **Timeout**: tiempo máximo de espera de la llamada al webhook.
...
#### `CLIENT_CERTIFICATE_AND_EXTERNAL_WEBHOOK`

Este modo combina los dos controles anteriores durante `/simpleenroll`: el cliente debe presentar un certificado válido por mTLS y, además, el webhook externo debe devolver `authorized: true`. En la práctica, conviene leer este modo como la suma de las reglas descritas en los apartados `CLIENT_CERTIFICATE` y `EXTERNAL_WEBHOOK`.

En `/simplereenroll` esta combinación no se conserva como tal. El flujo de renovación no mantiene el chequeo conjunto mTLS + webhook, por lo que el integrador no debe asumir que las mismas garantías del enrollment inicial se aplican automáticamente al reenroll.
...
- **Reglas importantes de re-enrollment**: una regla crítica para el integrador es la **inmutabilidad del subject**. Lamassu compara, byte a byte, el `RawSubject` del CSR con el `RawSubject` del certificado en vigor (`slices.Compare`). Si los bytes difieren, el servidor cae a una comparación semántica de respaldo sobre los atributos concatenados por comas: `CommonName`, `OrganizationalUnit`, `Organization`, `Locality`, `Province`, `Country`. Si todos esos atributos coinciden a nivel shallow, el reenroll continúa pese a la diferencia de bytes. Esto suele rescatar casos donde solo cambia la codificación (`PrintableString` frente a `UTF8String`). Si algún atributo difiere, el reenroll se aborta con `invalid RawSubject bytes`.
  ...
  El servidor también valida la cadena del certificado presentado. La validación se intenta primero contra la CA de enrolamiento del DMS; si falla, recorre las `AdditionalValidationCAs` hasta encontrar una emisora válida. La validación es "shallow": cada CA se prueba como ancla, no se compone una cadena multinivel. Por eso, el cliente debe presentar la cadena completa en el handshake mTLS. El certificado se comprueba además contra OCSP/CRL: si está revocado, el reenroll se aborta con `certificate is revoked`.
