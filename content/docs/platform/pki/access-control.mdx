...
description: Conecta una identidad OIDC o X.509 con principales, políticas y permisos de Lamassu.
---
...
2. Envoy envía la petición al endpoint de autorización externa /v1/ext_authz/check.
...
- **oidc**: coincide con claims de un token, por ejemplo un rol, grupo o <code>preferred_username</code>.
...
Fastlane instala un realm de Keycloak para laboratorio y crea el usuario <code>lamassu</code> con contraseña temporal <code>lamassu</code>, rol <code>pki-admin</code> y acción obligatoria de cambio de contraseña. Su principal de bootstrap coincide por <code>preferred_username=lamassu</code>.
