...
description: Entiende qué eventos de auditoría genera Lamassu y cómo convertirlos en una pista durable.
---
...
- <code>has_error</code>: indica si el servicio devolvió un error;
...
<Steps>
  ...
  <Step>
    ### Atribuye el principal

    Revisa principals y correlaciónalo con el proveedor OIDC o la identidad X.509 vigente en ese momento.
  </Step>

  <Step>
    ### Compara intención y resultado

    Contrasta input, has_error y output. Un intento fallido no implica que el recurso haya cambiado.
  </Step>
