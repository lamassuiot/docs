---
title: Device enrollment with EST
description: EST protocol guide for Lamassu IoT, with DMS configuration for the PKI administrator and the endpoint reference for device integrators.
sidebar:
  group: RA
  label: EST
---
...
Lamassu IoT implements the **EST (Enrollment over Secure Transport)** protocol described in RFC 7030 as one of its device certificate-enrollment mechanisms — the other is [CMP (RFC 9483)](/docs/platform/pki/cmp). Each DMS exposes one of the two protocols, never both at once. The DMS that exposes EST validates the device and applies its own authentication, registration and renewal policies — acting as the Registration Authority (RA) facing the client.
