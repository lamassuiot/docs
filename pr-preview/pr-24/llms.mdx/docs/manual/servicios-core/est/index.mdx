# EST: Enrollment over Secure Transport (/docs/pr-preview/pr-24/manual/servicios-core/est)



EST: Enrollment over Secure Transport [#est-enrollment-over-secure-transport]

Lamassu IoT implementa el protocolo **EST (Enrollment over Secure Transport)** descrito en el RFC 7030 como uno de sus mecanismos de inscripción de certificados para dispositivos — el otro es [CMP (RFC 9483)](/docs/manual/servicios-core/cmp). Cada DMS expone uno de los dos protocolos, nunca ambos a la vez; el que expone EST valida al dispositivo y aplica sus propias políticas de autenticación, registro y renovación — actuando como autoridad de registro (RA) frente al cliente.
