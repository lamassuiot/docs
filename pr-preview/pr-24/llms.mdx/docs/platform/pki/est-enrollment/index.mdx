# Device enrollment with EST (/docs/pr-preview/pr-24/platform/pki/est-enrollment)



EST: Enrollment over Secure Transport [#est-enrollment-over-secure-transport]

Lamassu IoT implements the **EST (Enrollment over Secure Transport)** protocol described in RFC 7030 as <del className="lm-diff-del">its only</del><ins className="lm-diff-ins">one of its device</ins> certificate<del className="lm-diff-del"> </del><ins className="lm-diff-ins">-</ins>enrollment <del className="lm-diff-del">mechanism for devices. Each DMS exposes the EST endpoints, </del><ins className="lm-diff-ins">mechanisms — the other is </ins>[<ins className="lm-diff-ins">CMP (RFC 9483)</ins>](/docs/platform/pki/cmp)<ins className="lm-diff-ins">. Each DMS exposes one of the two protocols, never both at once. The DMS that exposes EST </ins>validates the device and applies its own authentication, registration and renewal policies<del className="lm-diff-del">. In doing so, it acts</del><ins className="lm-diff-ins"> — acting</ins> as the Registration Authority (RA) facing the client.
...
<Cards>
  ...
  <div className="lm-diff-ins lm-diff-block">
    <Card title="CMP enrollment" href="/docs/platform/pki/cmp" />
  </div>
